...
def server_aggregate(global_model, client_weights):
    """
    Coordinate-wise median aggregation for unknown attackers.
    """
    global_dict = global_model.state_dict()
...
# def server_aggregate_median(global_model, client_weights):
#     global_dict = global_model.state_dict()
#     num_clients = len(client_weights)

#     for k in global_dict.keys():
#         param_list = torch.stack([client_weights[i][k] for i in range(num_clients)], dim=0)
#         if param_list.dtype.is_floating_point:
#             # Compute the median across clients
#             median_param, _ = torch.median(param_list, dim=0)
#             global_dict[k] = median_param
#         else:
#             # For integer parameters, use the value from the first client
#             global_dict[k] = param_list[0]
#     global_model.load_state_dict(global_dict)
#     return global_model
...
# Training without malicious attackers:
...
#         global_model = server_aggregate_median(global_model, client_weights)
...
# Training with simulated malicious attacks:
def federated_training(num_rounds, num_clients, client_epochs, device):
    global_model = ConvNet().to(device)
    train_dataset, test_dataset = load_datasets()
    client_datasets = partition_dataset(train_dataset, num_clients)
    ...
    num_malicious = np.random.randint(1, num_clients)
    malicious_clients = np.random.choice(range(num_clients), num_malicious, replace=False)
    ...
    for round_num in range(num_rounds):
        ...
        for client_num in range(num_clients):
            ...
            # Simulate attack if client is malicious
            if client_num in malicious_clients:
                attack_type = np.random.choice(['sign_flip', 'add_noise', 'random'])
                for key in client_state:
                    if client_state[key].dtype.is_floating_point:
                        if attack_type == 'sign_flip':
                            client_state[key] = -client_state[key]
                        elif attack_type == 'add_noise':
                            noise = torch.randn_like(client_state[key]) * 5
        ...
        # Aggregate client models using robust aggregation
        global_model = server_aggregate(global_model, client_weights)
...
def main():
    ...
    print("\nTraining completed.")
    torch.save(global_model.state_dict(), 'federated_mnist_median_attack_model.pth')
